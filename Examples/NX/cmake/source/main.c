...
#ifndef SWITCH_PYTHON_HOME
#define SWITCH_PYTHON_HOME "romfs:/python"
#endif
...
__attribute__((used)) size_t __stacksize__ = SWITCH_PYTHON_STACK_SIZE;
...
main(int argc, char *argv[])
{
    ...
    printf("Config home...\n");
    consoleUpdate(NULL);
    status = PyConfig_SetBytesString(&config, &config.home, SWITCH_PYTHON_HOME);
    if (print_python_status(status)) {
        PyConfig_Clear(&config);
        goto wait_for_exit;
    }
    ...
    status = append_bytes_path(&config.module_search_paths, SWITCH_PYTHON_STDLIB);
    if (print_python_status(status)) {
        PyConfig_Clear(&config);
        goto wait_for_exit;
    }
    status = append_bytes_path(&config.module_search_paths, SWITCH_PYTHON_EXAMPLES);
    if (print_python_status(status)) {
        PyConfig_Clear(&config);
        goto wait_for_exit;
    }
