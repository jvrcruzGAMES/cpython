...
if pkg_config_exists libcurl; then
    ...
    echo "switch-curl was not found by pkg-config; switch_ssl will report curl as unavailable."
fi
...
# Switch SSL/backend support module. This is generated by Platforms/NX/build-portlibs.sh.
*static*
switch_ssl switch_ssl.c $switch_support_defines $switch_support_cflags $switch_support_libs
EOF
...
make -C "$nx_build_dir" $install_dest libinstall inclinstall libainstall
...
for hacl_lib in \
    ...
    if [ -f "$nx_build_dir/Modules/_hacl/$hacl_lib" ]; then
        cp "$nx_build_dir/Modules/_hacl/$hacl_lib" \
...
if [ -f "$embed_pc" ]; then
    sed -i "s|^Libs.private:.*|Libs.private: $embed_private_libs|" "$embed_pc"
fi

rm -rf "$runtime_root/lib"
mkdir -p "$runtime_root/lib"
cp -R "$install_prefix/lib/python$version" "$runtime_root/lib/"
rm -rf \
...
cat > "$install_prefix/share/python-switch/README.md" <<EOF
